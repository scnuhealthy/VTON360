"""
This is an Extended Version of [NeRF Studio's Splatfacto](https://docs.nerf.studio/nerfology/methods/splat.html).
"""
...
@dataclass
class SplatfactoxModelConfig(ModelConfig):
    """Splatfactox Model Config"""

    _target: Type = field(default_factory=lambda: SplatfactoxModel)
    ...
    max_opacity_loss_scale: float = 1.0 
    """max_opacity_loss_scale to force opacity in masked area to zero."""



class SplatfactoxModel(Model):
    """An Extended Version of Splatfacto

    Args:
        config: Splatfactox configuration to instantiate model
    """

    config: SplatfactoxModelConfig
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if not isinstance(camera, Cameras):
            ...
            return {}
        assert camera.shape[0] == 1, f"Only one camera at a time, got {camera.shape[0]}"
        ...
        ######## render opacity map ########
        opacity_map = rasterize_gaussians(
            self.xys,
            depths,
            self.radii,
            conics,
            num_tiles_hit,
            # Note that we use ones for rgb here, and the rendered result would be cumulated opacity.
            torch.ones_like(rgbs),
            torch.sigmoid(opacities_crop),
            H,
            W,
            background=torch.zeros_like(background), # note that we use zeros.
        )  # type: ignore
        ####### render opacity map ########
        ...
        return {"rgb": rgb, "depth": depth_im, "opacity": opacity_map}  # type: ignore
    ...
    # opacity loss for the masked region
    def opacity_loss(self, outputs, mask):
        max_opacity_loss_scale = self.config.max_opacity_loss_scale

        # opacity loss scale is linearly growing with training step and clipped at 1.0 
        opacity_loss_scale = max_opacity_loss_scale * min((self.step / self.config.stop_split_at), 1.0)

        return opacity_loss_scale * torch.abs(
                    (outputs["opacity"] - torch.zeros_like(outputs["opacity"])) * (~mask)
                ).mean()
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        if "mask" in batch:
            # add [:2] to the mask, since some masks have a singleton dimension for channel while others not. 
            assert batch["mask"].shape[:2] == gt_img.shape[:2] == pred_img.shape[:2]
            mask = batch["mask"].to(self.device)
            # mask = batch["mask"][..., None].to(self.device) # uncomment this line if your mask has no dimension for channel.

            # we set groudtruth of the background as bg_color used in alpha-blending. 
            gt_img = self.background_color.to(self.device)[None, None, :] * (~mask) + gt_img * mask

        Ll1 = torch.abs(gt_img - pred_img).mean()
        ##### add opacity loss of the masked area #####
        Ll1 = Ll1 + self.opacity_loss(outputs, mask)
